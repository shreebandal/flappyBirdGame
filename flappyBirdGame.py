...
GROUNDY = SCREENHEIGHT * 0.9 - 22
GAME_SPRITES = {}
GAME_SOUNDS = {}
PLAYER = 'img/bird.png'
BACKGROUND = 'img/flappy.png'
PIPE = 'img/pipe.png'
    ...
    while True:
        for event in pygame.event.get():
            if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
                ...
                SCREEN.blit(GAME_SPRITES['base'], (GAME_SPRITES['base'].get_width(), GROUNDY))
...
def isCollide(playerx, playery, upperPipes, lowerPipes):
    if playery > GROUNDY - 25 or playery < 0:
        SCREEN.blit(GAME_SPRITES['explosion'],(playerx-10,GROUNDY-50))
        pygame.display.update()
    ...
    for pipe in upperPipes:
        ...
        if (playery < pipeHeight + pipe['y'] and abs(playerx - pipe['x']) < GAME_SPRITES['pipe'][0].get_width()):
            SCREEN.blit(GAME_SPRITES['explosion'], (playerx-10,playery-30))
            pygame.display.update()
            GAME_SOUNDS['hit'].play()
            return True
...
def getRandomPipe():
    ...
    y2 =  random.randrange(250,460)
    pipeX = SCREENWIDTH + 10
    y1 = pipeHeight - y2 + offset/1.5
    pipe = [
        {'x': pipeX, 'y': -y1},  
        {'x': pipeX, 'y': y2}  
    ]
...
if __name__ == "__main__":
    ...
    GAME_SPRITES['explosion'] = pygame.image.load('img/explosion1.gif').convert_alpha()
